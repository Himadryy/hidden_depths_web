// --- UPDATED: Part 0: Animated Intro Logic ---
window.addEventListener('load', () => {
    const introOverlay = document.getElementById('intro-overlay');
    const introLogo = document.getElementById('intro-logo');
    const brandNameContainer = document.querySelector('.brand-name-container');
    const mainContent = document.getElementById('main-content');
    
    // Sequence Step 2: After 2 seconds, pop out logo and trigger letter assembly
    setTimeout(() => {
        introLogo.classList.add('pop-out');
        brandNameContainer.classList.add('visible');
    }, 2000);

    // Sequence Step 3: After 4.5 seconds total, fade out the entire intro
    // (Increased duration to allow letter animation to finish)
    setTimeout(() => {
        introOverlay.style.opacity = '0';
        
        setTimeout(() => {
            introOverlay.style.display = 'none';
        }, 1500); // Matches the slower CSS transition
...
const orbMaterial = new THREE.ShaderMaterial({
    vertexShader: `
        ...
        float snoise(vec3 v) { 
            ...
            vec4 x_o = mod(x_, 7.0); vec4 y_o = mod(y_, 7.0);
            vec4 h = 1.0 - abs(x_) / 49.0 - abs(y_) / 49.0;
            vec4 b0 = vec4(x_o.x, x_o.y, y_o.x, y_o.y); vec4 b1 = vec4(x_o.z, x_o.w, y_o.z, y_o.w);
        ...
        void main() { 
            vNormal = normal; 
            float displacement = snoise(normal + time * 0.05) * 0.15;
...
const starMaterial = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        void main() {
            ...
            vec3 darkColor = vec3(0.87, 0.72, 0.45); 
