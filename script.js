...
const renderer = new THREE.WebGLRenderer({
    ...
    alpha: true // Important for background if we want body background to show
});
...
// The Morphing Orb (unchanged - it provides the white background)
const orbGeometry = new THREE.IcosahedronGeometry(5.5, 32);
const orbMaterial = new THREE.ShaderMaterial({
    vertexShader: `
        ...
        float snoise(vec3 v) { const vec2 C = vec2(1.0/6.0, 1.0/3.0); const vec4 D = vec4(0.0, 0.5, 1.0, 2.0); vec3 i = floor(v + dot(v, C.yyy)); vec3 x0 = v - i + dot(i, C.xxx); vec3 g = step(x0.yzx, x0.xyz); vec3 l = 1.0 - g; vec3 i1 = min(g.xyz, l.zxy); vec3 i2 = max(g.xyz, l.zxy); vec3 x1 = x0 - i1 + C.xxx; vec3 x2 = x0 - i2 + C.yyy; vec3 x3 = x0 - D.yyy; i = mod289(i); vec4 p = permute(permute(permute(i.z+vec4(0.,i1.z,i2.z,1.))+i.y+vec4(0.,i1.y,i2.y,1.))+i.x+vec4(0.,i1.x,i2.x,1.)); float n_ = 0.142857142857; vec3 ns = n_ * D.wyz - D.xzx; vec4 j = p - 49.0 * floor(p * ns.z * ns.z); vec4 x_ = floor(j * ns.z); vec4 y_ = floor(j - 7.0 * x_); vec4 x_o = mod(x_, 7.0); vec4 y_o = mod(y_, 7.0); vec4 h = 1.0 - abs(x_) / 49.0 - abs(y_) / 49.0; vec4 b0 = vec4(x_o.x, x_o.y, y_o.x, y_o.y); vec4 b1 = vec4(x_o.z, x_o.w, y_o.z, y_o.w); vec4 s0 = floor(b0)*2.0 + 1.0; vec4 s1 = floor(b1)*2.0 + 1.0; vec4 sh = -step(h, vec4(0.0)); vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy; vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww; vec3 p0 = vec3(a0.xy,h.x); vec3 p1 = vec3(a0.zw,h.y); vec3 p2 = vec3(a1.xy,h.z); vec3 p3 = vec3(a1.zw,h.w); vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2,p2), dot(p3,p3))); p0 *= norm.x; p1 *= norm.y; p2 *= norm.z; p3 *= norm.w; vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0); m = m * m; return 42.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3))); }
            void main() { vNormal = normal; float displacement = snoise(normal + time * 0.1) * 0.3; vec3 newPosition = position + normal * displacement; gl_Position = projectionMatrix * modelViewMatrix * vec4(newPosition, 1.0); }
        `,
    fragmentShader: `
        varying vec3 vNormal;
        void main() {
            float intensity = pow(0.8 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 3.0);
            gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0) * intensity; // Pure white for blending
        }
    `,
...
// The "Intelligent" Starfield (now with two colors)
const starVertices = [];
for (let i = 0; i < 15000; i++) { // Increased star count for a denser field
...
const starMaterial = new THREE.ShaderMaterial({
    ...
    `,
    fragmentShader: `
        uniform vec3 orbScreenPos; // Orb position in screen space (x,y, radius)
        uniform float orbScreenRadius;
        uniform float time; // for animated stars within orb
        
        void main() {
            // Dark mode (default) golden color
            vec3 darkColor = vec3(0.87, 0.72, 0.45); // E0B873 in RGB [0-1]
            
            // Light mode golden color (can be slightly different for effect)
            vec3 lightColor = vec3(1.0, 0.85, 0.6); // Slightly brighter gold
            
            // Calculate distance to orb center on screen
            float distToOrbCenter = distance(gl_FragCoord.xy, orbScreenPos.xy);
            
            // Determine if the fragment is inside the bright part of the orb
            // We use smoothstep to create a soft edge for the transition
            // The 50.0 is a tolerance for the orb's visible glow/radius
            float orbEffectStrength = 1.0 - smoothstep(orbScreenRadius - 70.0, orbScreenRadius + 70.0, distToOrbCenter);
            
            // Mix the colors based on whether it's in the orb's light area
            vec3 finalColor = mix(darkColor, lightColor, orbEffectStrength);
            
            // We can even make stars inside the orb slightly brighter or animate them differently
            // For now, they just change color.
            
            gl_FragColor = vec4(finalColor, 1.0); // Full opacity
        }
    `,
    uniforms: {
        orbScreenPos: { value: new THREE.Vector3() },
        orbScreenRadius: { value: 0.0 },
        time: { value: 0.0 } // Passed for potential future star animation within orb
    },
    transparent: true,
    // blending: THREE.AdditiveBlending // Removed AdditiveBlending for better control over the effect
});
const stars = new THREE.Points(starGeometry, starMaterial);
scene.add(stars);


// --- Part 3: The Unified Animation Loop ---
...
function animate() {
    ...
    // Calculate orb's screen position and radius and pass to star shader
    // We need the orb's *current* position and its rendered radius on screen
    const orbWorldPosition = new THREE.Vector3().copy(orb.position);
    
    // Project orb's center to screen coordinates
    ...
    // Estimate orb's screen radius (this is a bit tricky with perspective)
    // Project two points: center and one point on edge, then calculate screen distance
    ...
    starMaterial.uniforms.time.value = elapsedTime; // Update time for stars too
...
// --- Part 4: Scroll & Intro Animations ---
...
function updateScrollAnimations() {
    ...
    sectionsToAnimate.forEach(section => {
        ...
        const maxBlur = 8; const scrollFactor = distanceFromCenter / (viewportHeight / 2);
        const blurValue = scrollFactor * maxBlur; const opacityValue = 1 - Math.pow(scrollFactor, 1.5);
        section.style.filter = `blur(${blurValue}px)`; section.style.opacity = Math.max(0, opacityValue);
    });
}
