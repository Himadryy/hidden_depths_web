...
export default function Carousel() {
    ...
    const getSlideStyles = (i: number) => {
        ...
        const baseClasses = "absolute left-1/2 top-1/2 -translate-x-1/2 transition-all duration-700 ease-in-out";

        if (position === 0) {
            return `${baseClasses} z-20 scale-110 opacity-100 blur-0 -translate-y-1/2`; // Active (Center)
        } else if (position === total - 1) {
            return `${baseClasses} z-10 scale-90 opacity-40 blur-[2px] -translate-y-[80%]`; // Previous (Up)
        } else if (position === 1) {
            return `${baseClasses} z-10 scale-90 opacity-40 blur-[2px] -translate-y-[20%]`; // Next (Down)
        } else {
    ...
    return (
        <div className="w-full min-h-screen flex flex-col md:flex-row items-center justify-center gap-12 py-20">
            
            {/* Left: Text Content */}
            <div className="w-full md:w-1/2 text-left space-y-6 px-4 md:px-0 relative h-[300px] flex flex-col justify-center">
            ...
            {/* Right: 3D Card Stack */}
            <div className="w-full md:w-1/2 h-[500px] relative perspective-[1000px]">
                {slidesData.map((slide, i) => (
                    <div
                        key={slide.id}
                        className={`w-[280px] h-[400px] rounded-2xl overflow-hidden shadow-2xl border border-white/10 ${getSlideStyles(i)}`}
                    >
                        {slide.type === 'video' ? (
                            <video 
                                src={slide.imageUrl} 
