...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login to open the authentication modal (use exact path /login on current site).
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Fill the email and password fields and click the 'SIGN IN' button to attempt login (perform inputs for index 194 and 195, then click index 196).
        ...
        await expect(frame.locator('text=Total paid bookings').first).to_be_visible(timeout=3000)
