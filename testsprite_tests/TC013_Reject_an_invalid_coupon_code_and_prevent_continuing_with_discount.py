...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (explicit navigate per test step) to open the auth modal/login page.
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Type the email into the email input (index 97) and then type the password and submit the form.
        ...
        frame = context.pages[-1]
        assert '/' in frame.url
        await expect(frame.locator('text=Invalid coupon').first).to_be_visible(timeout=3000)
