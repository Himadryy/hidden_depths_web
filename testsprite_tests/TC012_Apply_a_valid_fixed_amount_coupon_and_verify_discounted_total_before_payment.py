...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (http://localhost:3000/login) to open the authentication form.
        await page.goto("http://localhost:3000/login", wait_until="commit", timeout=10000)
        
        # -> Type the provided credentials into the email and password fields and click SIGN IN (use indexes 273, 274, then click 275).
        ...
        # -> Click the 'BOOK AN INTRODUCTORY CALL' button (index=556) to open the booking flow and proceed toward applying the coupon.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[2]/main/section/div[2]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Click the date button for 'Sunday, March 8' (index=765) to open the booking details and proceed toward the coupon input step.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div[2]/div[2]/div/div/div/div/div[3]/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        
        # -> Open the booking details/next step so the coupon input becomes visible (click the relevant continue/next button). Then locate the Coupon code field and apply 'FLAT50'.
        frame = context.pages[-1]
        # Click element
